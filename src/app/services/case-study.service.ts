...
export class CaseStudyService extends BaseService {
  ...
  getCaseStudyReportFormValues(reportId: string, formId: string) {
    return this.get(`${this.url}/Report/${reportId}/${formId}`);
  }
  ...
  customExportReport(data: any): Observable<HttpResponse<Blob>> {
      return this.httpClient.post<Blob>(`${this.baseUrl+this.url}/CustomExport`, data, { observe: 'response', responseType: 'blob' as 'json' });
  }
  ...
  saveObservationRecord(payload: any) {
    return this.post('/Observation/Multi', payload);
  }
}
